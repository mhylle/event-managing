    ...
    describe('createUser', function () {
        describe('Success', function () {
            it('Should create a user', function () {
                $httpBackend.expectPOST('/api/user/').respond(
                    {status: 'ok', info: '', user: users.users[0]}
                );
                var userResult = [];
                userservice.createUser(users.users[0]).then(function (results) {
                    userResult = results.user;
                });
                $httpBackend.flush();
                expect(userResult).to.exists;
            ...
            it('Should not be able to create a user that already exists', function () {
                var user1;
                var user2;
        ...
        describe('Failure', function () {
            it('Should log an error if the server returns an error', function () {
                $httpBackend.expectPOST('/api/user/').respond(500);
                var userResult = [];
                userservice.createUser(users.users[0]).then(function (results) {
                    userResult = results;
                });
